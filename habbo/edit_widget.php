...
if($linked > 0){
	...
	if($num > 0){
		...
		if($linked > 0){
			mysql_query("UPDATE homes_stickers SET skin = '".$skin."' WHERE groupid = '".$groupid."' AND type = '2' AND id = '".$widget."' LIMIT 1");
		} else {
			mysql_query("UPDATE homes_stickers SET skin = '".$skin."' WHERE userid = '".$my_id."' AND groupid = '-1' AND type = '2' AND id = '".$widget."' LIMIT 1");
		}
