...
$remote_ip = SacarIP(); //$_SERVER[REMOTE_ADDR];

$remote_ip = $_SERVER['REMOTE_ADDR'];
$sitename = "HabPvP";
$shortname = "PvP";

if(@ini_get('date.timezone') == null && function_exists("date_default_timezone_get")){ @date_default_timezone_set("America/Mexico_City"); }
...
$path = $hotel_url;
$adminpath = mysql_real_escape_string("".$path."/manage/hotel/de/housekeeping");
$clientpath = $hotel_url;
$cimagesurl = $cimages_url;
$badgesurl = "http://beta.javvo.net/c_images/album1584/";
...
$maintenance = mysql_num_rows($maintenance = mysql_query("SELECT * FROM cms_settings WHERE variable = 'client_ip' AND value = '1'"));
...
// #########################################################################
// ADITTIONAL FUNCTIONS BY BARCEWIN
// #########################################################################
...
function FaceLikes() //Personas que les gusta barcewin en Facebook
	{
		$get = file_get_contents("http://www.facebook.com/barcewinofficial");
		$likes = @explode('<span class="uiNumberGiant fsxxl fwb">', $get);
		$likes = @explode('</span> personas les ', $likes[1]);
		$likes = trim($likes[0]);
		return $likes;
	
	}
function TwittSegui() //Seguidores de barcewin en Twitter
	{
		$get = file_get_contents("http://api.twitter.com/1/users/show/barcewin.json?callback=showUser");
		$segui = @explode('"followers_count":', $get);
		$segui = @explode(',"profi', $segui[1]);
		$segui = trim($segui[0]);
		return $segui;
...
############## VERSION ####################
$version = "barcewinCMS Private Edition";
############## VERSION ####################
