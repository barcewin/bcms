...
if(is_numeric($groupid) && $groupid > 0){
	...
	if(mysql_num_rows($check) > 0){
		...
		if(mysql_num_rows($check2) > 0){
			...
			mysql_query("UPDATE group_members SET is_current = '0' WHERE id_user = '".$my_id."'");
			mysql_query("UPDATE group_members SET is_current = '1' WHERE id_user = '".$my_id."' AND id_group = '".$groupid."' LIMIT 1");
